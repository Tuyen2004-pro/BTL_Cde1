...
<div class="card-cafe">
    ...
    <div class="card-body-cafe" style="padding:0">
        ...
        <div class="empty-state">
            ...
            <p>Chưa có đơn hàng nào. <a href="{{ route('staff.orders.create') }}" style="color:var(--caramel)">Tạo đơn
                    đầu tiên</a></p>
        </div>
        @else
            ...
            <table class="table-cafe">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <span
                                style="font-family:'Playfair Display',serif;font-weight:700;color:var(--caramel)">#{{ $order->id }}</span>
                        </td>
                        <td>
                        ...
                        <td style="text-align:center">
                            <div style="display:flex;align-items:center;justify-content:center;gap:6px;flex-wrap:wrap">
                                ...
                                <form action="{{ route('staff.orders.pay', $order->id) }}" method="POST"
                                    ...
                                    @csrf
                                    <button type="submit"
                                        style="background:#d1fae5;color:#065f46;border:1.5px solid #6ee7b7;border-radius:8px;padding:5px 10px;font-size:.78rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:4px">
                                        <i class="bi bi-credit-card"></i> TT
                                    </button>
                                </form>
                                @endif
                                <form id="del-ord-{{ $order->id }}"
                                    action="{{ route('staff.orders.destroy', $order->id) }}" method="POST">
                                    @csrf @method('DELETE')
                                </form>
                                <button onclick="confirmDelete('del-ord-{{ $order->id }}')" class="btn-danger-cafe"
                                    style="font-size:.78rem;padding:5px 10px">
