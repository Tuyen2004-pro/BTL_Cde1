...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:24px">
    ...
    <a href="{{ route('orders.create') }}" class="btn-espresso">
...
<div class="card-cafe">
    <div class="card-header-cafe">
        <h5><i class="bi bi-receipt me-2" style="color:var(--caramel)"></i>Danh sách đơn hàng</h5>
    </div>
    <div class="card-body-cafe" style="padding:0">
        @if($orders->isEmpty())
        <div class="empty-state">
            <i class="bi bi-receipt"></i>
            <p>Chưa có đơn hàng nào. <a href="{{ route('orders.create') }}" style="color:var(--caramel)">Tạo đơn đầu tiên</a></p>
        </div>
        @else
        <div style="overflow-x:auto">
            <table class="table-cafe">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            <span style="font-family:'Playfair Display',serif;font-weight:700;color:var(--caramel)">#{{ $order->id }}</span>
                        </td>
                        <td>
                            <div style="display:flex;align-items:center;gap:6px">
                                <span style="font-size:1rem">🪑</span>
                                <span style="font-weight:500">{{ $order->table?->name ?? 'Không có' }}</span>
                            </div>
                        </td>
                        ...
                        <td>
                            <div style="font-size:.8rem;color:rgba(26,14,5,.55);max-width:180px">
                                ...
                                <div style="color:var(--caramel)">+{{ $order->items->count() - 2 }} món khác</div>
                                @endif
                            </div>
                        </td>
                        <td>
                        ...
                        <td style="text-align:center">
                            <div style="display:flex;align-items:center;justify-content:center;gap:6px;flex-wrap:wrap">
                                <a href="{{ route('orders.show', $order->id) }}" class="btn-outline-cafe" style="padding:5px 10px;font-size:.78rem">
                                    <i class="bi bi-eye"></i> Xem
                                </a>
                                @if($order->status === 'pending')
                                <form action="{{ route('orders.pay', $order->id) }}" method="POST" style="display:inline">
                                    @csrf
                                    <button type="submit" style="background:#d1fae5;color:#065f46;border:1.5px solid #6ee7b7;border-radius:8px;padding:5px 10px;font-size:.78rem;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:4px">
                                        <i class="bi bi-credit-card"></i> TT
                                    </button>
                                </form>
                                @endif
                                <form id="del-ord-{{ $order->id }}" action="{{ route('orders.destroy', $order->id) }}" method="POST">
                                    @csrf @method('DELETE')
                                </form>
                                <button onclick="confirmDelete('del-ord-{{ $order->id }}')" class="btn-danger-cafe" style="font-size:.78rem;padding:5px 10px">
